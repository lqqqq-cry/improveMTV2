...
// Function to process records for a given index
void processRecordsForIndex(ddtacontext& context, SQLHDBC& dbc, int& index,
                            int& counter, bool& flag, 
                            SQLCHAR* SQLStmt, SQLSMALLINT& ret, char* szData, SQLHSTMT& stmt
                            );

// Mutex for thread-safe access to shared data structures like bimap and prediction.
std::mutex mtx;

// Algorithm to produce predictions
void producePrediction(ddtacontext& context, SQLHDBC& dbc) {
    ...
    // Create threads for each iteration of the loop
    for (int i = 0; i <= 3; ++i) { //作用在维表上的SQL
		// Define a lambda function to wrap the function call with its arguments
		auto threadFunc = [&](ddtacontext& context, void*& dbc) {
			processRecordsForIndex(context, dbc, i, counter, flag, SQLStmt, ret, szData, stmt);
		};

		// Use the lambda function with std::thread constructor
		threads.emplace_back(threadFunc, std::ref(context), std::ref(dbc));
    ...
    std::cout << "Threads exiting" << std::endl;
...
void processRecordsForIndex(ddtacontext& context, SQLHDBC& dbc, int& index,
    ...
    char* SqlString = context.rewrtSQLStat[index];

    if (strlen(SqlString) == 0) return;
    ...
    bimap<string, int>& bimap = index == 0 ? context.mapForCustomer :
        (index == 1 ? context.mapForSupplier :
            (index == 2 ? context.mapForDwdate :
                context.mapForPart
                )
            );

    vector<int>& prediction = index == 0 ? context.predCustomer :
        (index == 1 ? context.predSupplier :
            (index == 2 ? context.predDwdate :
                context.predPart
                )
            );

    dynamic_bitset<>& predbit = index == 0 ? context.predbitCust :
        (index == 1 ? context.predbitSupp :
            (index == 2 ? context.predbitDate :
                context.predbitPart
                )
            );

    //initialize for "0"
    if (context.isTranslated[index]) {
        ...
        while ((ret = SQLFetch(stmt)) == SQL_SUCCESS) {
            ...
            if (iter == bimap.left.end()) {//not in the map (if attribute not in the hash table)
                // Lock the mutex before modifying shared data structures
                std::lock_guard<std::mutex> lock(mtx);

                bimap.left.insert(make_pair(szData, counter)); //pair(CANADA,2)
                
                prediction.push_back(counter); //push back key into the key vecotor;

                // Print bimap after insertion
                std::cout << "Value of bimap after insertion: " << std::endl;
